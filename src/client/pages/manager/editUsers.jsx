...
function ListUsers() {
    ...
    useEffect(() => {
        if(users){
            setRoles(users.map((user) => user.role)
                ...
                {formOpen ?
                    <div>
                        <form onSubmit={handleUserUpdate}>
                            <label> New role:
                                <select defaultValue={"Select a role"} onChange={event => setSelectedRole(event.target.value)}>
                            ...
                            {NewRoleOpen ? <input type={"text"} placeholder={"new role"} onChange={event => setSelectedRole(event.target.value)}/> : null}
